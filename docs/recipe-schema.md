...
- GitHub Release cbox dependencies require `github = "owner/repository"` and a
  ...
  variant suffix.
  Use `forge upgrade [dependency] --latest` or
  `forge upgrade <dependency> --to=<version>` to update recipe versions and
  refresh locks in one step. Without a dependency name, `--latest` upgrades
  every selected direct GitHub dependency. `--all-targets` refreshes concrete
  targets already present in `forge.lock.toml`; `--release-targets` refreshes
  Forge's standard release matrix: `linux-x86_64`, `macos-arm64`, and
  `windows-x86_64`.
