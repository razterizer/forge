...
Forge offers to clone the vcpkg ports tree into `.forge/vcpkg`, install the
`vcpkg` executable through Homebrew if necessary, and then runs the manifest
install using that checkout. This keeps CMake packages such as `Stb` on their
declared vcpkg source rather than guessing a different system package.
