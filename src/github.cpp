  ...
  namespace
  {
                                  ...
                                  std::string_view compatibility)
    {
      return
        ...
        "            case \"$filename\" in\n"
        "              *-linux)\n"
        "                cp \"$archive\" \"hosted-assets/${filename}-" + std::string { compatibility }.substr(std::string_view { "linux-" }.size()) + ".zip\"\n"
        "                ;;\n"
        "              *)\n"
    ...
    std::string linux_release_workflow()
    {
      return
        ...
        + "  release:\n"
          ...
          "          path: linux-legacy\n"
          "\n"
          "      - name: Assemble Linux release assets\n"
          "        run: |\n"
          ...
          "            legacy=\"linux-legacy/${base}-legacy.zip\"\n"
          "            [ -e \"$legacy\" ] || continue\n"
          "            rm -rf linux-bundle\n"
          "            mkdir -p \"linux-bundle/${base}-modern\" \"linux-bundle/${base}-legacy\"\n"
          "            unzip -q \"$archive\" -d \"linux-bundle/${base}-modern\"\n"
          "            unzip -q \"$legacy\" -d \"linux-bundle/${base}-legacy\"\n"
