  ...
  bool print_command_help(std::string_view command, std::ostream& output)
  {
    if (command == "adopt")
    {
      output
        ...
        << "                         component initializes build.number and dotted\n"
        << "                         release numbering\n"
    ...
    if (command == "doctor")
    {
      output
        ...
        << "  --search-github   Search GitHub for unresolved dependency includes\n"
        << "  --local-search=<mode>\n"
        << "                    Local dependency search: nearby or off; default: nearby\n\n"
    ...
    if (command == "build")
    {
      output
        ...
        << "Options:\n"
        << "  --style=<name>       Select a dependency style (overrides [defaults].style):\n"
        << "                       local-source, git-source, local-package,\n"
        ...
        << "  --profile=<name>     Select a software profile (overrides [defaults].profile;\n"
        << "                       legacy recipes select a combined profile)\n"
